...
【スタイル・表現の絶対ルール】
**「視覚的に見やすいデザイン」**を意識して作成してください。

1. **日付見出し**: 日付ごとの区切りは \`## 🗓 1/13 (火)\` のような形式で出力してください。
2. **リスト形式**: 箇条書き記号（・や-）は使わず、必ず「意味に合った絵文字」を行頭に使ってください。
   - 例:
     ✅ 遊技台戦略: ...
     ℹ️ 強化機種: ...
     ⚠️ 注意事項: ...
3. **重要アラート**: 特に強い警告やルールは \`> \` (引用) で囲ってください。
4. **強調**: 重要なキーワードは \`**\` で囲んでください。
5. **トーン**: 丁寧かつ簡潔に。

【内容の振り分け】
- **short (全体要約版)**:
    - 未来の日付が含まれる**すべてのスケジュール**を網羅してください。（4日制限などは撤廃）
    - ただし、各項目の内容は「要点のみ」に絞り、パッと見て全体の流れがわかるようにしてください。
    - 詳細な戦略解説は省いてください。
- **full (詳細解説版)**:
    - shortの内容に加え、各スケジュールの背景にある「戦略的意図」や「具体的なアクションプラン」を詳しく解説してください。
    - 読み手が深く理解するための資料です。
